...
enum class OrderDestination(val wireValue: String) {
    TAKE_AWAY("para_llevar");

    companion object {
        fun fromWireValue(value: String): OrderDestination = entries.firstOrNull {
            it.wireValue == value
        } ?: throw IllegalArgumentException("destino no soportado en VAI-10: $value")
    }
}

enum class OrderState(val wireValue: String) {
    PENDING_PAYMENT("por_cobrar");

    companion object {
        fun fromWireValue(value: String): OrderState = entries.firstOrNull {
            it.wireValue == value
        } ?: throw IllegalArgumentException("estado no soportado en VAI-10: $value")
    }
}
