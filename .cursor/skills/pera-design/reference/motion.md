...
For deeper animation craft — frequency gating, review checklists, spring interruptibility, gesture velocity, vocabulary, Apple-fluid patterns — load `reference/emil/` (start with `README.md`). This table stays the page-budget source of truth; Emil's standards win when reviewing or fixing a specific transition that Pera does not name.
