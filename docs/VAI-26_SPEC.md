...
**Rama trabajo:** `cursor/vai-26-auth-checkout-4c06`  
...
VerifyEmail verified → completeEnrollment()
  → POST alta (MOCK/REMOTE) → sesiones/contexto → SecureSessionStore
...
## API REMOTE (placeholder hasta CONTRACTS Entrega-02 en repo)

Abstraer en `StudentEnrollmentApi`:

- `POST identidades/cliente/alta` con Bearer Firebase ID token  
  Body mínimo esperado: `establecimiento_id`, `identificador_contextual` (nullable), `nombre`, `aceptacion_terminos_en` (ISO UTC)

Respuesta: `membresia_id` para `sesiones/contexto`.
